# NCAA WBB Team Efficiency Landscape
...
![Example](outputs/ncaa_wbb_efficiency_2026_beat_shaded_simplelabels.png)

## Two scripts, run in order

| Script | What it does |
|---|---|
| `scripts/r/02_ncaa_wbb_efficiency_stat_grabber.R` | Pulls the season's team box scores from `wehoop`, pairs each team with its opponent per game, aggregates to season totals, and computes ORtg / DRtg / Net Rating. Writes a CSV. |
| `scripts/r/02.5_ncaa_wbb_efficiency_plot_beat_shaded_simplelabels.R` | Reads that CSV and draws the chart. |
...
**Season.** In the grabber, `load_wbb_team_box(seasons = 2026)` — `wehoop` uses
