###############################################
# NCAA WBB "New Pac-12" Pizza Plot (Dark Theme)
# Compare a player to the REST of the NEW Pac-12,
# within their position group -- e.g. Utah State
# guards vs. all new-Pac-12 guards.
#
# Template: 02_wbb/scripts/r/6.25_wbb_pizza_plot.R
# Data:     wehoop load_wbb_player_box (college stats)
#
# WHY THIS SCRIPT EXISTS: the rebuilt Pac-12 (2026-27
# on) is not yet a selectable conference in ESPN's
# conference/standings API, so we can't resolve its
# membership the way the template does. Instead we
# hardcode the member schools (CONFIG below) and resolve
# their team_ids by matching team names in the pulled
# player-box data.
#
# Note: all "*_pct" and `usage` are decimals (0-1)
#       *_pctile_pos columns are weighted percentiles
#       computed WITHIN the school pool and position
#       bucket (Guard / Forward / Center).
#       *_pos_avg = weighted mean of raw metric for that
#       position group inside the pool.
...
#
# HOW TO USE: edit the CONFIG block (section 0) to swap
# the player, school, member-school list, and season.
...
main_player     <- "Karyn Sanford"           # player to feature
school          <- "Utah State"              # the player's school (title only)
season          <- 2026                      # wehoop uses END year (2026 = 2025-26)

# Labels used on titles / legend / filenames.
conf_full_name  <- "Pac-12"
conf_short_name <- "Pac-12"

# Member schools of the new Pac-12. Matched (case-insensitively)
# against wehoop's team_display_name / team_short_display_name /
# team_location. Full "Location Nickname" strings are the most
# reliable; the script warns if any school fails to match.
pac12_schools <- c(
  "Boise State Broncos",
  "Colorado State Rams",
...
# before percentiles are computed. Leave commented to keep
# everyone. Uncomment section in 5b as well # filter(minutes_total >= min_minutes) %>%
# NOTE: the featured player must clear this too.
# min_minutes <- 100
...
###############################################
# 5a. Resolve the new-Pac-12 schools -> team_ids
#     by matching names in the pulled data.
#     (Replaces the ESPN conference API in the template.)
###############################################

# Distinct teams present in this season's data
team_lookup <- wbb_player_season %>%
  distinct(team_id, team_display_name, team_short_display_name, team_location)

# Match each configured school against display/short/location names
match_school <- function(s) {
  team_lookup %>%
    filter(
      team_display_name       == s |
      team_short_display_name == s |
      team_location           == s |
      grepl(s, team_display_name, ignore.case = TRUE)
    )
}

matched  <- lapply(pac12_schools, match_school)
n_hits   <- vapply(matched, nrow, integer(1))

# Warn on schools that didn't match or matched ambiguously
if (any(n_hits == 0)) {
  message("These configured schools matched NO team in the ", season,
          " data and will be dropped:")
  print(pac12_schools[n_hits == 0])
  message("Available team_display_name values (browse to fix the CONFIG list):")
  print(sort(team_lookup$team_display_name))
}
if (any(n_hits > 1)) {
  message("These configured schools matched MULTIPLE teams; using the first each:")
  for (i in which(n_hits > 1)) {
    message("  '", pac12_schools[i], "' ->")
    print(matched[[i]])
  }
}

conf_teams <- bind_rows(lapply(matched, function(m) if (nrow(m)) m[1, ] else m)) %>%
  distinct(team_id, .keep_all = TRUE)

conf_team_ids <- as.character(conf_teams$team_id)

message(conf_full_name, " pool resolved: ", nrow(conf_teams),
        " of ", length(pac12_schools), " configured schools")
print(conf_teams$team_display_name)

###############################################
# 5b. Restrict to the school pool, then compute
#     weighted percentiles + position averages
#     WITHIN pool + position bucket
###############################################
...
wbb_conf_season <- wbb_player_season %>%
  ...
  group_by(pos_group) %>%
  mutate(
    # --- Percentiles (within pool + position) ---
...
wbb_conf_season_lab <- wbb_conf_season %>%
  transmute(
    season, athlete_id,
...
###############################################
# 6.5 Print player vs pool position avg
###############################################

library(knitr)

# Find the featured player inside the school pool
player_row <- wbb_conf_season_lab %>% filter(player == main_player)

if (nrow(player_row) == 0) {
  message("Player '", main_player, "' not found in the ", conf_full_name,
          " pool. Players on ", school, ":")
  wbb_conf_season_lab %>%
...
# Pool position-group average for per-game stats (unweighted mean)
pos_avg <- wbb_conf_season_lab %>%
  filter(pos_group == player_row$pos_group[1]) %>%
  summarise(across(c(mpg, ppg, rpg, apg, spg, bpg, tovpg),
                   ~round(mean(., na.rm = TRUE), 2))) %>%
  mutate(player = paste0(conf_short_name, " ",
                         player_row$pos_group[1], " Avg"))

# Player row + pool position avg side by side
bind_rows(
...
wbb_conf_season_lab <- wbb_conf_season_lab %>%
...
conf_slug <- gsub("[^a-z0-9]+", "_", tolower(conf_short_name))
...
league     <- "02_wbb"   # league folder this script belongs to
...
write_csv(
  wbb_conf_season_lab,
  file = file.path(data_dir, paste0("wbb_", conf_slug, "_", season, "_lab.csv"))
)
...
LETTER_COLOR     <- "white"
AVG_BAR_COLOR    <- "white"    # pool-average ghost-bar overlay
...
# --- 8.3b Prepare pool position-average ring data ---
...
pizza_plot_dark <- ggplot(
  ...
  # Pool-average ghost-bar overlay
  geom_bar(
    aes(y = AvgPctile),
    width = 0.85,
    stat = "identity",
...
final_plot <- final_plot +
  ...
  cowplot::draw_label(
    paste0("Percentile Profile vs. ", conf_short_name, " ",
  ...
  cowplot::draw_label(
    paste0(conf_short_name, " ", pos_label, " Avg (shaded)"),
